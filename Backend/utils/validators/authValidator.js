...
const signupRules=[
    check("username").notEmpty().withMessage("username is required").isLength({ min: 2, max: 32 }).withMessage("username must be between 2 and 32 characters"),
    check("email").notEmpty().withMessage("email is required").isEmail().withMessage("wrong email format").custom(async(val)=>{
